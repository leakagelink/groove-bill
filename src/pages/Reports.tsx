...
export default function Reports() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Sales Register Tab - day-wise grouping */}
      {tab === 'sales_register' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-muted-foreground">{filteredSales.length} entries • Total: ₹{salesTotal.toLocaleString('en-IN')}</p>
            <Button size="sm" variant="outline" onClick={() => downloadCSV('sales')}><Download size={14} className="mr-1" /> Export CSV</Button>
          </div>
          <div className="bg-card rounded-lg border overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-left p-3 font-medium text-muted-foreground">Sr No</th>
                  <th className="text-left p-3 font-medium text-muted-foreground">Date</th>
              ...
              <tbody className="divide-y">
                {filteredSales.length === 0 && (
                  <tr><td colSpan={8} className="p-4 text-center text-muted-foreground">No sales found</td></tr>
                )}
                {[...filteredSales].sort((a, b) => a.date.localeCompare(b.date)).map((s, i) => (
                  <tr key={s.id} className="hover:bg-muted/30">
                    <td className="p-3 text-muted-foreground">{i + 1}</td>
                    <td className="p-3 text-foreground">{new Date(s.date).toLocaleDateString('en-IN')}</td>
                    <td className="p-3 font-mono text-primary text-xs">{s.invoiceNumber}</td>
                    <td className="p-3 font-medium text-foreground">{s.customerName}</td>
                    <td className="p-3 text-muted-foreground capitalize">{s.paymentMethod || 'cash'}</td>
                    <td className="p-3 text-right text-muted-foreground">₹{s.totalAmount.toLocaleString('en-IN')}</td>
                    <td className="p-3 text-right text-destructive">₹{s.totalDiscount.toLocaleString('en-IN')}</td>
              ...
              {filteredSales.length > 0 && (
                <tfoot>
                  <tr className="border-t bg-muted/30 font-semibold">
                    <td colSpan={5} className="p-3 text-foreground">Total</td>
                    <td className="p-3 text-right text-foreground">₹{filteredSales.reduce((s, i) => s + i.totalAmount, 0).toLocaleString('en-IN')}</td>
                    <td className="p-3 text-right text-destructive">₹{salesDiscount.toLocaleString('en-IN')}</td>
                    <td className="p-3 text-right text-foreground">₹{salesTotal.toLocaleString('en-IN')}</td>
                  </tr>
                </tfoot>
              )}
            </table>
      ...
      {tab === 'purchase_register' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-muted-foreground">{filteredPurchases.length} entries • Total: ₹{purchaseTotal.toLocaleString('en-IN')}</p>
            <Button size="sm" variant="outline" onClick={() => downloadCSV('purchases')}><Download size={14} className="mr-1" /> Export CSV</Button>
          </div>
          <div className="bg-card rounded-lg border overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {filteredPurchases.length === 0 && (
                  <tr><td colSpan={7} className="p-4 text-center text-muted-foreground">No purchases found</td></tr>
                )}
                {[...filteredPurchases].sort((a, b) => a.date.localeCompare(b.date)).map((p, i) => (
                  <tr key={p.id} className="hover:bg-muted/30">
                    <td className="p-3 text-muted-foreground">{i + 1}</td>
                    <td className="p-3 text-foreground">{new Date(p.date).toLocaleDateString('en-IN')}</td>
                    <td className="p-3 font-mono text-xs text-primary">{p.id.slice(0, 8).toUpperCase()}</td>
                    <td className="p-3 font-medium text-foreground">{p.supplierName}</td>
                    <td className="p-3 text-muted-foreground capitalize">{p.paymentMethod || 'cash'}</td>
                    <td className="p-3 text-right text-muted-foreground">{p.items.length}</td>
                    <td className="p-3 text-right font-medium text-foreground">₹{p.totalAmount.toLocaleString('en-IN')}</td>
              ...
              {filteredPurchases.length > 0 && (
                <tfoot>
                  <tr className="border-t bg-muted/30 font-semibold">
                    ...
                    <td className="p-3 text-right text-foreground">₹{purchaseTotal.toLocaleString('en-IN')}</td>
                  </tr>
                </tfoot>
              )}
            </table>
      ...
      {tab === 'payment_register' && (() => {
        const payments: Array<{
          date: string; type: 'Receipt' | 'Payment'; party: string;
          method: string; refNo: string; amount: number;
        }> = [];
        filteredSales.filter(s => s.paymentStatus === 'paid').forEach(s => {
          payments.push({
            date: s.date, type: 'Receipt', party: s.customerName,
            method: s.paymentMethod || 'cash', refNo: s.invoiceNumber,
            amount: s.finalAmount,
          });
        });
        filteredPurchases.forEach(p => {
          payments.push({
            date: p.date, type: 'Payment', party: p.supplierName,
            method: p.paymentMethod || 'cash', refNo: p.id.slice(0, 8).toUpperCase(),
            amount: p.totalAmount,
          });
        });
        payments.sort((a, b) => a.date.localeCompare(b.date));
        const receiptsTotal = payments.filter(p => p.type === 'Receipt').reduce((s, p) => s + p.amount, 0);
        const paymentsTotal = payments.filter(p => p.type === 'Payment').reduce((s, p) => s + p.amount, 0);
        return (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-card rounded-lg border p-4">
                <div className="text-xs text-muted-foreground">Receipts (In)</div>
                <p className="text-xl font-bold text-green-600 mt-1">₹{receiptsTotal.toLocaleString('en-IN')}</p>
              </div>
              <div className="bg-card rounded-lg border p-4">
                <div className="text-xs text-muted-foreground">Payments (Out)</div>
                <p className="text-xl font-bold text-destructive mt-1">₹{paymentsTotal.toLocaleString('en-IN')}</p>
              </div>
              <div className="bg-card rounded-lg border p-4">
                <div className="text-xs text-muted-foreground">Net Cashflow</div>
                <p className={`text-xl font-bold mt-1 ${(receiptsTotal - paymentsTotal) >= 0 ? 'text-green-600' : 'text-destructive'}`}>
                  ₹{(receiptsTotal - paymentsTotal).toLocaleString('en-IN')}
                </p>
              </div>
            </div>
            <div className="bg-card rounded-lg border overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="text-left p-3 font-medium text-muted-foreground">Ref No</th>
                    <th className="text-right p-3 font-medium text-muted-foreground">Amount</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {payments.length === 0 && (
                    <tr><td colSpan={6} className="p-4 text-center text-muted-foreground">No payments found</td></tr>
                  )}
                  {payments.map((p, i) => (
                    <tr key={i} className="hover:bg-muted/30">
                      <td className="p-3 text-foreground">{new Date(p.date).toLocaleDateString('en-IN')}</td>
                      <td className="p-3">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${p.type === 'Receipt' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                          {p.type}
                        </span>
                      </td>
                      <td className="p-3 font-medium text-foreground">{p.party}</td>
                      <td className="p-3 text-muted-foreground capitalize">{p.method}</td>
                      <td className="p-3 font-mono text-xs text-primary">{p.refNo}</td>
                      <td className={`p-3 text-right font-medium ${p.type === 'Receipt' ? 'text-green-600' : 'text-destructive'}`}>
      ...
      {/* Ledger Report Tab */}
      {tab === 'ledger' && (() => {
        type LedgerEntry = { date: string; particulars: string; type: string; debit: number; credit: number; refNo: string };
        const entries: LedgerEntry[] = [];

        if (ledgerAccount !== 'all') {
          filteredSales.filter(s => s.customerName === ledgerAccount).forEach(s => {
            entries.push({
              date: s.date, particulars: `Sales Invoice ${s.invoiceNumber}`,
              type: 'Sale', debit: s.finalAmount, credit: 0, refNo: s.invoiceNumber,
            });
            if (s.paymentStatus === 'paid') {
              entries.push({
                date: s.date, particulars: `Receipt against ${s.invoiceNumber} (${s.paymentMethod || 'cash'})`,
                type: 'Receipt', debit: 0, credit: s.finalAmount, refNo: s.invoiceNumber,
              });
            }
          });
          filteredPurchases.filter(p => p.supplierName === ledgerAccount).forEach(p => {
            entries.push({
              date: p.date, particulars: `Purchase Bill ${p.id.slice(0, 8).toUpperCase()}`,
              type: 'Purchase', debit: 0, credit: p.totalAmount, refNo: p.id.slice(0, 8).toUpperCase(),
            });
            entries.push({
              date: p.date, particulars: `Payment for ${p.id.slice(0, 8).toUpperCase()} (${p.paymentMethod || 'cash'})`,
              type: 'Payment', debit: p.totalAmount, credit: 0, refNo: p.id.slice(0, 8).toUpperCase(),
            });
          });
        }
        ...
        return (
          <div className="space-y-4">
            <div className="bg-card rounded-lg border p-4">
              ...
              <select className="flex h-10 w-full sm:w-96 rounded-md border border-input bg-background px-3 py-2 text-sm mt-1" value={ledgerAccount} onChange={e => setLedgerAccount(e.target.value)}>
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="bg-card rounded-lg border p-4">
                    ...
                    <p className="text-xl font-bold text-foreground mt-1">₹{totalDebit.toLocaleString('en-IN')}</p>
                  </div>
                  <div className="bg-card rounded-lg border p-4">
                    <div className="text-xs text-muted-foreground">Total Credit</div>
                    <p className="text-xl font-bold text-foreground mt-1">₹{totalCredit.toLocaleString('en-IN')}</p>
                  </div>
                  <div className="bg-card rounded-lg border p-4">
                    <div className="text-xs text-muted-foreground">Closing Balance</div>
                    <p className={`text-xl font-bold mt-1 ${closingBal >= 0 ? 'text-green-600' : 'text-destructive'}`}>
                      ₹{Math.abs(closingBal).toLocaleString('en-IN')} {closingBal >= 0 ? 'Dr' : 'Cr'}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-card rounded-lg border overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        <th className="text-left p-3 font-medium text-muted-foreground">Date</th>
                        <th className="text-left p-3 font-medium text-muted-foreground">Particulars</th>
                        <th className="text-left p-3 font-medium text-muted-foreground">Type</th>
                        <th className="text-left p-3 font-medium text-muted-foreground">Ref No</th>
                    ...
                    <tbody className="divide-y">
                      {withBalance.length === 0 && (
                        <tr><td colSpan={7} className="p-4 text-center text-muted-foreground">No transactions found for this account</td></tr>
                      )}
                      {withBalance.map((e, i) => (
                        <tr key={i} className="hover:bg-muted/30">
                          <td className="p-3 text-foreground">{new Date(e.date).toLocaleDateString('en-IN')}</td>
                          <td className="p-3 text-foreground">{e.particulars}</td>
                          <td className="p-3 text-muted-foreground">{e.type}</td>
                          <td className="p-3 font-mono text-xs text-primary">{e.refNo}</td>
                          <td className="p-3 text-right text-muted-foreground">{e.debit > 0 ? `₹${e.debit.toLocaleString('en-IN')}` : '-'}</td>
                          <td className="p-3 text-right text-muted-foreground">{e.credit > 0 ? `₹${e.credit.toLocaleString('en-IN')}` : '-'}</td>
                          <td className={`p-3 text-right font-medium ${e.balance >= 0 ? 'text-foreground' : 'text-destructive'}`}>
                    ...
                    {withBalance.length > 0 && (
                      <tfoot>
                        <tr className="border-t bg-muted/30 font-semibold">
                          ...
                          <td className="p-3 text-right text-foreground">₹{totalDebit.toLocaleString('en-IN')}</td>
                          <td className="p-3 text-right text-foreground">₹{totalCredit.toLocaleString('en-IN')}</td>
                          <td className={`p-3 text-right ${closingBal >= 0 ? 'text-foreground' : 'text-destructive'}`}>
                            ₹{Math.abs(closingBal).toLocaleString('en-IN')} {closingBal >= 0 ? 'Dr' : 'Cr'}
                          </td>
                        </tr>
                      </tfoot>
